...
const Friends = () => {
    ...
    const fetchUserId = async (token) => {
        try {
            const response = await axios.get('http://teeput.synology.me:30112/ms3/mypage', { params: { token } });
    ...
    const handleAddFriend = async (friendId) => {
        const token = localStorage.getItem('token');
        try {
            const response = await axios.post('http://teeput.synology.me:30112/ms3/friend/add', { userId, friendId, status: 'pending' }, {
    ...
    const handleAcceptRequest = async (friendId) => {
        const token = localStorage.getItem('token');
        try {
            const response = await axios.put('http://teeput.synology.me:30112/ms3/friend/accept', { userId: friendId, status: 'accepted' }, {
                headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
                params: { token }
            });
            if (response.data.status === 'success') {
